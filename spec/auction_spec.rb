...
RSpec.describe Auction do
    ...
    describe '#bidder_info' do
        it 'returns a hash with keys that are attendees' do
            @auction.add_item(@item1)
            @auction.add_item(@item2)
            @auction.add_item(@item3)
            @auction.add_item(@item4)
            ...
            expect(@auction.bidder_info.keys.length).to eq(3)
        end

        it 'returns a hash with values that are hashes with attendees budgets and items they have bid on' do
            @auction.add_item(@item1)
            @auction.add_item(@item2)
            @auction.add_item(@item3)
            @auction.add_item(@item4)
            ...
            expect(@auction.bidder_info.values.length).to eq(3)
        end
    end
end
